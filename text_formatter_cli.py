...
if __name__ == "__main__":
    ...
    print(f"  选项 2: 顺序格式 (使用 \"{SEQUENTIAL_MARKER_START}项目{SEQUENTIAL_MARKER_END}\" 包裹，例如: {SEQUENTIAL_MARKER_START}项1{SEQUENTIAL_MARKER_END}{SEQUENTIAL_MARKER_START}项2{SEQUENTIAL_MARKER_END})")
    print("-" * 30)
    ...
    try:
        ...
        if len(sys.argv) == 3:
            ...
            while True:
                input_str = input("请输入原始文本 (多个项目用空格分隔，例如: 张三 李四 王五): ").strip()
                if not input_str:
                    print("输入为空，退出程序。")
                    break
