...
PAYMENT_QR_FILENAME = "payment_ten.jpg"  # 支付二维码文件名 (虽然按钮移除，但图片路径可能仍被引用，保留定义)
...
class ActivationDialog(QDialog):
    ...
    # --- 移除或注释掉 _handle_paid_user_activation_request 方法 ---
    # def _handle_paid_user_activation_request(self):
    ...
    #     self._process_activation_attempt()
    ...
    def _process_activation_attempt(self):  # 处理激活码输入和验证
        ...
        if isinstance(activation_data_entry, str):
        ...
        if validity_code_after_activation == "UNL":
            calculated_expiry = issue_timestamp + (365 * 20 * 24 * 60 * 60)
